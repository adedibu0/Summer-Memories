...
export const updateMediaItem = (userId: string, mediaId: string, updates: Partial<MediaItem>): MediaItem => {
  const mediaItems = getMediaItems(userId)
...
export const getMediaItemsByCategory = (userId: string, category: string): MediaItem[] => {
  const mediaItems = getMediaItems(userId)
  return mediaItems.filter((item) => item.categories.includes(category) || category === "all")
}

export const suggestGroups = async (userId: string): Promise<{ name: string; items: string[] }[]> => {
  const mediaItems = getMediaItems(userId)
